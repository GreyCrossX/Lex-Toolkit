...
import apps.agent.research_graph as rg


def test_build_query_text_uses_issue_and_facts():
    step = {"issue_id": "I1", "layer": "law", "description": "desc", "status": "pending", "query_ids": []}
    state = {
        "issues": [
            {
                "id": "I1",
...
def test_run_next_search_step_with_stub_tool(monkeypatch):
    ...
    state = {
        "issues": [
            {"id": "I1", "question": "Pregunta", "priority": "high", "area": "labor", "status": "pending"}
        ],
        "research_plan": [
            {
                "id": "R1",
