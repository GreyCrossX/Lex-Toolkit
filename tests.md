...
- **Frontend (Next.js)**  
  - Unit: utilidades y hooks (pendiente agregar tests con Jest/RTL).  
  - Integración: páginas con Playwright/Cypress (pendiente).  
...
```bash
# Desde la raíz
pipenv run pytest           # ejecuta unit + integración + e2e placeholder
# Con stack vivo (futuro): pipenv run pytest -m "not e2e"  # o incluir e2e cuando haya seed/compose
```

## Próximos pasos
- Backend: agregar tests de éxito para `/search` y `/qa` usando un pool de test o fixtures de pgvector (requiere seed).
- Frontend: añadir Jest/RTL para hooks y Playwright para flujos básicos (login demo → dashboard → search).
