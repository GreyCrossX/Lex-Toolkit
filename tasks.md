...
- GRE-29 Search interface UI — Local: In review (dashboard search/QA form hits `/search` and `/qa`, filters/pagination/sorting + Vitest coverage; search mode now expects `results` payload from `/search`) / Linear: Move to In Progress (was Todo).
- GRE-30 Upload document UI — Local: In progress (dashboard upload calls `/upload`/`/ingestion/{doc_type}`, shows filename/status) / Linear: Move to In Progress (was Todo).
...
- GRE-34 Summary UI page — Local: In progress (dashboard summary UI calls `/summary`, renders citations, placeholder fallback) / Linear: Move to In Progress (was Todo).
- GRE-51 Polish search UI — Local: Done (QA/search toggle, filters, pagination, distance sorting, Vitest; `/search` endpoint reflected in UI) / Linear: Move to In Progress (was Backlog).
- GRE-52 Wire upload UI to ingestion — Local: Done (backend ingestion live with `/upload` + `/ingestion/{doc_type}`, status polling via `ingestion_jobs`) / Linear: Move to In Progress (was Backlog).
- GRE-53 Connect summary UI to /summary — Local: In progress (frontend wired; needs backend responses and streaming support) / Linear: Move to In Progress (was Backlog).
...
- Auth hardening (no ticket yet): Done locally — access JWT + rotated refresh tokens stored server-side; endpoints `/auth/register|login|refresh|logout|me|health` live; hashing via pbkdf2 + jose HS256 signing. Not yet enforcing auth on `/search|/qa|/upload*`, no cookies/rate-limits/RS256/CSRF yet. TODO: add enforcement, HttpOnly cookie flow for frontend, auth integration tests, and key rotation. Needs Linear issue.
...
2) Ingestion hardening: add specialized parsers per `doc_type` (jurisprudence/contract/policy) and richer metadata; keep timeouts small. Extend dashboard upload UX as needed (GRE-30/52).
